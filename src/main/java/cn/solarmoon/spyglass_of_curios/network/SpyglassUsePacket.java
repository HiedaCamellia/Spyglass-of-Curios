    ...
    public static void handle(SpyglassUsePacket packet, Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            ...
            switch (packet.Handle) {
                ...
                case "soundUse" -> {
                    level.playSound(null, player.getOnPos().above(), SoundEvents.SPYGLASS_USE, SoundSource.PLAYERS, 1f, 1f);
                }
                case "soundStop" -> {
                    level.playSound(null, player.getOnPos().above(), SoundEvents.SPYGLASS_STOP_USING, SoundSource.PLAYERS, 1f, 1f);
                }
            }
        });
